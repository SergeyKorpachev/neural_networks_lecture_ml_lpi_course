...
\begin{frame}
    ...
    \centering
    \vspace{1.5cm}
	\includegraphics[width=0.7\linewidth]{images/want_have.png}\\
    \begin{itemize}
        \centering
        \item {The linear model does not describe complex nonlinear data}
        \item {Need to use something non-linear}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            Regression:\\
            \includegraphics[width=0.5\linewidth]{images/plot.png}\\
            Classification:\\
            \includegraphics[width=0.5\linewidth]{images/4gi.png}\\
        \end{minipage}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \centering
    \vspace{1.7cm}
    \includegraphics[width=0.9\linewidth]{images/some_model.png}
    \begin{itemize}
        \item {Add a block to a linear model to automatically generate new features}
        \item {Two blocks (unknown model and linear model) must work and update parameters as a single model}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \centering
    \vspace{1.7cm}
    \includegraphics[width=0.9\linewidth]{images/lin_model.png}\\
    \begin{itemize}
        \item {Add a linear model to an unknown block}
        \item {$\hat{y} = w^{T} \cdot \tilde{x} = w^{T} \cdot (W \cdot x) = (w^{T} \cdot W) \cdot x = w'^{T} \cdot x \Rightarrow$ it is still a linear model}
        \item {The input feature space has not changed, only the weights have changed}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \centering
    \vspace{1.7cm}
    \includegraphics[width=0.9\linewidth]{images/add_nonlin.png}
    \begin{itemize}
        \item {Add nonlinearity to our model}
        \item {$\hat{y} = w^{T} \cdot \tilde{x} = w^{T} \cdot g(W \cdot x)$,\\
    where $g(\cdot)$ some nonlinear scalar function (applied elementwise)}
        \item {This is the simplest example of a neural network}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \centering
    \vspace{1.7cm}
    \includegraphics[width=0.9\linewidth]{images/nn_details.png}\\
    \begin{itemize}
        \centering
        \item {$\hat{y} = w^{T} \cdot \tilde{x} = w^{T} \cdot g(W \cdot x) = \sum\limits_{j=1}\big[ w_{j} \cdot g(\sum\limits_{i}W_{ji} \cdot x_{i}) \big]$}
        \item {NN is a function that maps a set of $X$ to a set of $Y$}
    \end{itemize}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{minipage}[t]{\linewidth}
            \begin{itemize}
                ...
                \item {$g(\cdot)$ - nonlinear activation function, for example, a sigmoid function $\sigma(\cdot)$}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item {How to find the partial derivative of the loss function with respect to some weight?}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item {Consider a simpler neural network}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item {We use forward pass to get the output of each neuron and compute the loss function to then back propagate the error}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item {We use backward pass to compute the gradients in the neural network and get updated weight for each neuron}
    \end{itemize}
\end{frame}

%------------------------------------------------

\subsection{NN training procedure}
\begin{frame}
    \myframetitle{0.45\paperwidth}{0.04\paperwidth}{\insertsection}
    \myframesubtitle{0.47\paperwidth}{\insertsubsection}
    \centering
    \vspace{1.7cm}
    \begin{itemize}
        \item {Firstly, we make a forward pass through the neural network to calculate the output of each neuron and find the value of the loss function.}
        \item {Secondly, we go back through the neural network to calculate the value of the gradients to update the weights later.}
        \item {Thirdly, we repeat the given procedure at a new iteration with updated weights.}
        \item {One iteration (epoch) is a forward and backward pass of the neural network.}
        \item {It is necessary to make as many epochs (with pre-configured neural network hyperparameters) until the model produces the desired result.}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
	    ...
	    \hfill \break
	    \item {Should the input features be on the same scale (normalization of the input data) or not? Why?}\\
	\end{itemize}
\end{frame}
